...
public class StudentService {
    ...
    public Page<Student_Response_DTO> getStudentDynamically(Student_Request_DTO req, Long schoolId ,Pageable pg) {
        if (req == null) {
            throw (new IllegalArgumentException("Invalid student search criteria!"));
        }

        Specification<Student> spec = StudentSpecification.searchStudent(req , schoolId);
    ...
    public Boolean AttendingCheck(Long studentId, Boolean Attended) {
        ...
        if (Attended == true) {
            ...
            student.setLastTrainingDate(LocalDateTime.now());
            System.out.println("Attending check success.");
        } else {
            student.setAttendanceStatus(AttendanceStatus.ABSENT);
            student.setNextTrainingDate(LocalDateTime.now().with(TemporalAdjusters.next(DayOfWeek.MONDAY)));
